## Release Notes

### New Features

- New default mode: **PROMPT IT** (free) to generate short, high-quality LLM prompts.
- Spotlight now supports quick pinned-mode switching with `Cmd/Ctrl + 1..9`.

### Bug Fixes

- Onboarding: on the “You’re all set” step, pressing the Spotlight shortcut now closes onboarding.
- Spotlight now opens with an empty input (removed automatic copy/paste behavior).
- Modes menu trigger (search/cross) behavior is now stable when opening/closing the menu.
- Pinned modes are hidden from the modes search palette to avoid duplicate entries.
